...
class Team(models.Model):
    ...
    country = models.CharField(
        max_length=70, choices=[(country, country) for country in COUNTRIES]
    )
    budget = models.DecimalField(max_digits=10, decimal_places=2, default=5000000)
    owner = models.OneToOneField(
        CustomUser, on_delete=models.CASCADE, related_name="team"
    )
...
class Player(models.Model):
    ...
    listing_status = models.CharField(
        max_length=20, choices=LISTING_STATUS_CHOICES, default="Not Listed"
    )
