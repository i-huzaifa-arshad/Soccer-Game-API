...
class BaseClassForUnitTest(APITestCase):
    def setUp(self):
        ...
        self.transfer_list = TransferList.objects.create(
            player=self.players[0], asking_price=Decimal("4500.00")
        )

        # Creating a transfer list instance for 2nd user
        self.transfer_list2 = TransferList.objects.create(
            player=self.players2[0], asking_price=Decimal("5500.00")
        )
...
class PlayerModelTest(BaseClassForUnitTest):
    ...
    def test_player_creation(self):
        ...
        self.assertIn(
            self.player.position, ["Goalkeeper", "Defender", "Midfielder", "Attacker"]
        )
...
class TransferListSerializerTest(BaseClassForUnitTest):
    ...
    def test_transfer_list_serializer(self):
        ...
        serializer = TransferListSerializer(
            instance=transfer_list, context={"players": self.players}
        )
        data = serializer.data

        self.assertEqual(
            data["player"],
            transfer_list.player.first_name + " " + transfer_list.player.last_name,
        )
...
class MarketListSerializerTest(BaseClassForUnitTest):
    ...
    def test_market_list_serializer(self):
        ...
        self.assertEqual(
            data["player_name"],
            f"{market_list.transfer_list.player.first_name} {market_list.transfer_list.player.last_name}",
        )
        self.assertEqual(
            data["player_country"], market_list.transfer_list.player.country
        )
        ...
        self.assertEqual(
            data["asking_price"], f"$ {market_list.transfer_list.asking_price}"
        )
...
class TeamUpdateViewTest(BaseClassForUnitTest):
    def setUp(self):
        super().setUp()  # Inheriting from the Base Class For Unit Test
        self.url = reverse(
            "team-update", kwargs={"owner__username": self.user.username}
        )
        self.token = Token.objects.create(user=self.user)
    ...
    def test_team_update_view(self):
        ...
        response = self.client.put(
            self.url,
            {"name": new_team_name, "country": new_team_country},
            format="json",
        )
...
class PlayerUpdateViewTest(BaseClassForUnitTest):
    def setUp(self):
        super().setUp()  # Inheriting from the Base Class For Unit Test
        self.player = (
            self.user.team.players.first()
        )  # Taking the first player of the team
        self.url = reverse(
            "player-update",
            kwargs={"teamname": self.user.team.name, "id": self.player.id},
        )
        self.token = Token.objects.create(user=self.user)
...
class TransferListViewTest(BaseClassForUnitTest):
    ...
    def test_transfer_list_view(self):
        ...
        self.assertTrue(
            MarketList.objects.filter(transfer_list__player=self.player).exists()
        )
...
class BuyPlayerViewTest(BaseClassForUnitTest):
    ...
    def test_buy_player_view(self):
        ...
        self.assertEqual(
            self.user.team.budget, self.buyer_initial_budget - self.asking_price
        )
        self.assertEqual(
            self.user2.team.budget, self.seller_initial_budget + self.asking_price
        )

        # Checking if the market value of the player has increased
        initial_market_value = self.player.market_value
        self.player.market_value = max(
            self.player.market_value * (1 + Decimal(randint(10, 100)) / 100),
            initial_market_value + Decimal("0.01"),
        )
        self.player.save()
        ...
        self.assertEqual(
            self.user.team.final_value,
            self.user.team.team_value + self.user.team.budget,
        )
        self.assertEqual(
            self.user2.team.final_value,
            self.user2.team.team_value + self.user2.team.budget,
        )
